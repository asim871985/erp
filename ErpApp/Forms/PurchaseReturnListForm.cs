...
/// <summary>Mirror of <see cref="PurchaseInvoiceListForm"/> for Purchase Returns.</summary>
public class PurchaseReturnListForm : AppFormBase
{
    ...
    private void BuildLayout()
    {
        ...
        btnNew.Click += (s, e) => OpenChild(new PurchaseReturnForm());
    ...
    private void LoadGrid()
    {
        ...
        {
            string sql = @"
                SELECT pr.return_id AS ""ID"", pr.return_no AS ""Return No."", pr.return_date AS ""Date"",
                       s.supplier_name AS ""Supplier"", pb.bill_no AS ""Against Bill"",
                       pr.total_amount AS ""Total Amount""
                FROM purchase_return pr
                LEFT JOIN supplier_master s ON s.supplier_id = pr.supplier_id
                LEFT JOIN purchase_bill pb ON pb.purchase_id = pr.purchase_id
            ...
            {
                sql += " AND (pr.return_no ILIKE @s OR s.supplier_name ILIKE @s)";
                pars["s"] = $"%{txtSearch.Text.Trim()}%";
            }
            sql += " ORDER BY pr.return_date DESC, pr.return_id DESC";
        ...
        {
            MessageBox.Show("Could not load purchase returns: " + ex.Message);
        }
    }
    ...
    private void EditSelected()
    {
        ...
        OpenChild(new PurchaseReturnForm(ids[0]));
    }
    ...
    private void DeleteSelected()
    {
        ...
        string msg = ids.Count == 1
            ? "Delete this purchase return? This reverses its stock and ledger effect."
            : $"Delete these {ids.Count} purchase returns? This reverses each one's stock and ledger effect.";
        ...
        try
        {
            foreach (var id in ids)
            {
                ...
                {
                    DbHelper.ReversePurchaseReturnPostings(conn, tx, id);
                    using var delCmd = new Npgsql.NpgsqlCommand("DELETE FROM purchase_return WHERE return_id=@id", conn, tx);
                    delCmd.Parameters.AddWithValue("id", id);
                    delCmd.ExecuteNonQuery();
                });
                DbHelper.LogAction($"Purchase Return: Deleted #{id}");
            }
            MessageBox.Show($"Deleted {ids.Count} purchase return(s).");
            LoadGrid();
        }
        catch (Exception ex)
        {
    ...
    private void PrintSelected()
    {
        ...
        if (ids.Count == 1)
        {
            using var printForm = new InvoicePrintForm(ids[0], PrintDocType.PurchaseReturn);
            printForm.ShowDialog(this);
        }
        else
        {
            BatchInvoicePrinter.PrintOrPreview(this, ids, PrintDocType.PurchaseReturn);
        }
    }
}
