...
/// <summary>Matches mockup form 2 "Purchase Return".</summary>
public class PurchaseReturnForm : AppFormBase
{
    ...
    private readonly ComboBox cboPurchaseInvoice = new() { DropDownStyle = ComboBoxStyle.DropDownList };
    ...
    public PurchaseReturnForm() : this(null) { }

    public PurchaseReturnForm(int? editReturnId)
    {
    ...
    private void BuildLayout()
    {
        ...
        L(3, 0, "Purchase Invoice"); C(3, 1, cboPurchaseInvoice);

        cboSupplier.SelectedIndexChanged += (s, e) => LoadPurchaseInvoicesForSupplier();
        ...
        btnBrowse.Click += (s, e) =>
        {
            var list = new PurchaseReturnListForm();
    ...
    private void LoadPurchaseInvoicesForSupplier()
    {
        cboPurchaseInvoice.DataSource = null;
        if (cboSupplier.SelectedValue is not int supId) return;

        try
        {
            var bills = DbHelper.ExecuteQuery(
                "SELECT purchase_id, bill_no FROM purchase_bill WHERE supplier_id=@id ORDER BY bill_date DESC",
                new() { ["id"] = supId });
            cboPurchaseInvoice.DisplayMember = "bill_no";
            cboPurchaseInvoice.ValueMember = "purchase_id";
            cboPurchaseInvoice.DataSource = bills;
        }
        catch (Exception ex)
        {
            MessageBox.Show("Could not load purchase invoices: " + ex.Message);
        }
    }
    ...
    private void NewDocNo()
    {
        try
        {
            var num = DbHelper.ExecuteScalar("SELECT prefix || LPAD(next_number::text, padding, '0') || COALESCE(suffix,'') FROM document_numbering WHERE doc_type='PURCHASE_RETURN'");
            txtReturnNo.Text = num?.ToString() ?? "PR-00001";
        }
    ...
    private void LoadForEdit(int returnId)
    {
        try
        {
            var header = DbHelper.ExecuteQuery("SELECT * FROM purchase_return WHERE return_id=@id", new() { ["id"] = returnId });
            if (header.Rows.Count == 0) { MessageBox.Show("That purchase return no longer exists."); NewDocNo(); return; }
            var h = header.Rows[0];
            ...
            if (h["purchase_id"] != DBNull.Value) cboPurchaseInvoice.SelectedValue = Convert.ToInt32(h["purchase_id"]);
            txtRemarks.Text = h["remarks"]?.ToString();

            lines.Clear();
            var items = DbHelper.ExecuteQuery(@"
                SELECT pri.item_id, i.item_name, i.model, u.uom_name, pri.qty, pri.rate, pri.disc_percent
                FROM purchase_return_item pri
                LEFT JOIN item_master i ON i.item_id = pri.item_id
                LEFT JOIN uom_master u ON u.uom_id = i.uom_id
                WHERE pri.return_id=@id ORDER BY pri.line_id", new() { ["id"] = returnId });
            ...
            RecalculateTotals();
            Text = $"Purchase Return — Editing {txtReturnNo.Text}";
        }
        catch (Exception ex)
        {
            MessageBox.Show("Could not load purchase return for editing: " + ex.Message);
        }
    }
    ...
    private void BtnSave_Click(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                decimal total = lines.Sum(l => l.Amount);
                int? purchaseId = cboPurchaseInvoice.SelectedValue as int?;
                ...
                if (isEditMode && currentReturnId != null)
                {
                    ...
                    DbHelper.ReversePurchaseReturnPostings(conn, tx, returnId);

                    using var updCmd = new Npgsql.NpgsqlCommand(@"
                        UPDATE purchase_return SET return_date=@dt, purchase_id=@pid, supplier_id=@sup,
                               remarks=@remarks, total_amount=@total
                        WHERE return_id=@id", conn, tx);
                    updCmd.Parameters.AddWithValue("dt", dtReturnDate.Value.Date);
                    updCmd.Parameters.AddWithValue("pid", (object?)purchaseId ?? DBNull.Value);
                ...
                foreach (var line in lines)
                {
                    ...
                    // Returning goods to the supplier reduces our stock
                    ...
                    using var stockCmd = new Npgsql.NpgsqlCommand(@"
                        ...
                        VALUES (@item, @wh, 'OUT', @qty, 'PURCHASE_RETURN', @r)", conn, tx);
                ...
                // Ledger: Debit supplier (payable down) / Credit Purchases (expense down)
                ...
                using var purchCmd = new Npgsql.NpgsqlCommand(@"
                    INSERT INTO ledger_entry (voucher_no, voucher_type, account_id, particulars, debit, credit, reference_id)
                    VALUES (@no, 'Purchase Return', @acc, 'By Purchase Return', 0, @total, @r)", conn, tx);
                purchCmd.Parameters.AddWithValue("no", returnNo);
                purchCmd.Parameters.AddWithValue("acc", purchaseAccountId);
                purchCmd.Parameters.AddWithValue("total", total);
                purchCmd.Parameters.AddWithValue("r", returnId);
                purchCmd.ExecuteNonQuery();
            ...
            DbHelper.LogAction($"Purchase Return: Saved {txtReturnNo.Text}");
    ...
    private void BtnDelete_Click(object? sender, EventArgs e)
    {
        ...
        if (MessageBox.Show($"Delete purchase return {txtReturnNo.Text}? This reverses its stock and ledger effect.",
        ...
        try
        {
            ...
            {
                DbHelper.ReversePurchaseReturnPostings(conn, tx, returnId);
                using var delCmd = new Npgsql.NpgsqlCommand("DELETE FROM purchase_return WHERE return_id=@id", conn, tx);
                delCmd.Parameters.AddWithValue("id", returnId);
                delCmd.ExecuteNonQuery();
            });
            DbHelper.LogAction($"Purchase Return: Deleted #{returnId}");
            MessageBox.Show("Purchase return deleted.");
            ResetForm();
        }
        catch (Exception ex)
        {
    ...
    private void BtnPrint_Click(object? sender, EventArgs e)
    {
        ...
        using var printForm = new InvoicePrintForm(currentReturnId.Value, PrintDocType.PurchaseReturn);
